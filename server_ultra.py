# server_ultra.py - Ultra-fast FastAPI server with parallel processing
from fastapi import FastAPI, HTTPException, BackgroundTasks
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import FileResponse
from pydantic import BaseModel
from typing import List, Optional
import asyncio
import uvicorn
from noah_core_ultra import noah_core_ultra
import os
from dotenv import load_dotenv

# Load environment variables
load_dotenv()

app = FastAPI(
    title="Daily Noah Ultra API",
    description="Ultra-fast AI-powered news bulletins with parallel processing",
    version="2.0.0"
)

# CORS middleware
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# Request models
class GenerateRequest(BaseModel):
    topics: List[str]
    language: str
    voice: str
    duration: int
    tone: str = "professional"
    strict_timing: bool = True
    quick_test: bool = False

class HealthResponse(BaseModel):
    openai: bool
    elevenlabs: bool
    tavily: bool
    ok: bool
    ultra_mode: bool = True

@app.get("/health")
async def health_check():
    """Ultra-fast health check"""
    try:
        # Quick health checks
        openai_status = bool(os.getenv("OPENAI_API_KEY"))
        elevenlabs_status = bool(os.getenv("ELEVENLABS_API_KEY"))
        tavily_status = bool(os.getenv("TAVILY_API_KEY"))
        
        return HealthResponse(
            openai=openai_status,
            elevenlabs=elevenlabs_status,
            tavily=tavily_status,
            ok=all([openai_status, elevenlabs_status, tavily_status]),
            ultra_mode=True
        )
    except Exception as e:
        return HealthResponse(
            openai=False,
            elevenlabs=False,
            tavily=False,
            ok=False,
            ultra_mode=True
        )

@app.get("/voices")
async def get_voices():
    """Get available voices with ultra-fast caching"""
    try:
        voices = await noah_core_ultra.get_available_voices_ultra()
        return voices
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error fetching voices: {str(e)}")

@app.post("/generate")
async def generate_bulletin(request: GenerateRequest, background_tasks: BackgroundTasks):
    """Generate ultra-fast news bulletin"""
    try:
        # Validate request
        if not request.topics:
            raise HTTPException(status_code=400, detail="Topics are required")
        
        if request.duration < 1 or request.duration > 15:
            raise HTTPException(status_code=400, detail="Duration must be between 1 and 15 minutes")
        
        print(f"🚀 Ultra-fast generation request: {len(request.topics)} topics, {request.duration} minutes")
        
        # Generate bulletin with ultra-fast processing
        result = await noah_core_ultra.make_noah_audio_ultra(
            topics=request.topics,
            language=request.language,
            voice_id=request.voice,
            duration=request.duration,
            tone=request.tone,
            strict_timing=request.strict_timing,
            quick_test=request.quick_test
        )
        
        if result.get("status") == "success":
            # Add cleanup task
            background_tasks.add_task(cleanup_resources)
            return result
        else:
            raise HTTPException(status_code=500, detail=result.get("error", "Unknown error"))
            
    except HTTPException:
        raise
    except Exception as e:
        print(f"❌ Error in ultra-fast generation: {e}")
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")

@app.get("/download/{name}")
async def download_audio(name: str):
    """Download generated audio file"""
    try:
        # This would serve actual audio files
        # For now, return a placeholder
        return {"message": f"Audio file {name} would be downloaded here"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error downloading audio: {str(e)}")

@app.get("/test-ultra")
async def test_ultra():
    """Test ultra-fast performance"""
    try:
        start_time = asyncio.get_event_loop().time()
        
        # Test parallel processing
        test_result = await noah_core_ultra.make_noah_audio_ultra(
            topics=["tech news", "AI developments"],
            language="English",
            voice_id="21m00Tcm4TlvDq8ikWAM",
            duration=3,
            tone="professional"
        )
        
        end_time = asyncio.get_event_loop().time()
        total_time = end_time - start_time
        
        return {
            "status": "success",
            "test_result": test_result,
            "performance": {
                "total_time_seconds": total_time,
                "ultra_mode": True,
                "parallel_processing": True,
                "caching_enabled": True
            }
        }
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Ultra test failed: {str(e)}")

async def cleanup_resources():
    """Cleanup resources after generation"""
    try:
        # This would clean up temporary files, etc.
        pass
    except Exception as e:
        print(f"Cleanup error: {e}")

@app.on_event("shutdown")
async def shutdown_event():
    """Cleanup on shutdown"""
    try:
        noah_core_ultra.cleanup()
    except Exception as e:
        print(f"Shutdown cleanup error: {e}")

if __name__ == "__main__":
    uvicorn.run(
        "server_ultra:app",
        host="0.0.0.0",
        port=int(os.getenv("PORT", 8000)),
        reload=False,  # Disable reload for production
        workers=1  # Single worker for now
    )
