...
def api_ok(): return API_BASE.startswith("http")
...
def call_api(payload:Dict[str,Any])->Dict[str,Any]:
    if not api_ok(): raise RuntimeError("API_BASE missing. Set it in Render → Environment.")
    r=requests.post(f"{API_BASE}/generate", json=payload, timeout=600); r.raise_for_status()
    j=r.json();
    if isinstance(j,dict) and j.get("error"): raise RuntimeError(j["error"])
    return j
...
def render_audio(mp3_url:str, actual_sec:float, target_min:int, rate_hint:float|None):
    ...
    if not mp3_url.startswith("http"): mp3_url=f"{API_BASE}{mp3_url}"
...
with st.sidebar:
    ...
    go=st.button("🚀 Generate Noah", use_container_width=True, key="go")

# Results
L,R=st.columns([0.5,0.5])
if go:
    queries=parse_topics(topics_raw)
    if not queries: st.error("Please add at least one topic."); st.stop()

    payload={"queries":queries,"language":language,"tone":tone,"recent_hours":int(lookback),
             "per_feed":6,"cap_per_query":int(cap), "min_minutes":int(minutes),
             "minutes_target":int(minutes), "exact_minutes":True}
    if voice_id and voice_id!="__custom__": payload["voice_id"]=voice_id

    with st.status("Generating your Noah… this can take 30–60 seconds", expanded=True) as s:
        t0=time.time()
        data=call_api(payload)
        s.write(f"Received in {time.time()-t0:.1f}s."); s.update(label="Done ✓", state="complete")
    ...
    with R:
        ...
        if not src: st.caption("No sources returned.")
        else:
